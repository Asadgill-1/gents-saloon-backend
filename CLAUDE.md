...
Phase 2 is active by explicit owner approval. T2.0–T2.5 are complete: operation sources, booking/queue, legal documents, cash shifts, checkout/payments, commission snapshots, balanced journal posting, and append-only correction reversals are locally/remotely verified. Continue at T2.6 advances, payout runs, and settlement. Phase 1 implementation is complete but its credential/repository-control/Storage audit gates remain open; never hide or mark them passed without evidence. Read [START_HERE.md](START_HERE.md), check [docs/REQUIREMENTS.md](docs/REQUIREMENTS.md), update the active phase checklist, and append durable decisions to [docs/PROJECT_CONTEXT.md](docs/PROJECT_CONTEXT.md).
