...
async def _write_event(
    ...
    *,
    ...
    actor_type = "auth_user" if actor_id is not None else "system"
    ...
    *,
    ...
    try:
        async with pool.connection(timeout=5) as connection, connection.transaction():
            ...
            queue_date = None
            queue_number = None
            status = "held" if payload.booking_type == "appointment" else "confirmed"
            ...
            cursor = await connection.execute(
                ...
                (
                    ...
                    hold_expires_at,
                    confirmed_at,
                    payload.booking_type != "appointment",
                    now,
                    now,
                ),
            )
