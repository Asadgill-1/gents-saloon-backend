...
**Security foundation, customer button flows, and transactional AI booking tools implemented; full phase incomplete.** A new forward migration and backend services now provide AES-256-GCM token envelopes, HMAC webhook-secret digests, durable update claims/deduplication/retry, database-derived role authorization, private-chat enforcement, aiogram transport, retrying outbox delivery, callback allowlisting, Redis-backed flood/AI budgets, strict AI tool schemas, redacted chat storage/retention, and authoritative rendering tests. Durable opaque customer callbacks now cover EN/AR/HI/UR language selection, service/barber/date/slot selection, requested queue creation, five-minute appointment holds and confirmation, own-booking views, cancellation/rescheduling, live queue, authoritative prices, and idempotent sanitized escalation. Allowlisted AI tools find the same server-derived appointment slots and call the existing transactional create, cancel, and reschedule services. Every mutation reauthorizes the Telegram customer inside its database transaction and derives idempotency from the trusted bot/update context rather than model arguments. The 17 applied Phase 1/2 migrations remain immutable.

The reception/barber/owner/master menus still acknowledge selections with a placeholder instead of executing every canonical flow; customer contact-share/name capture and notification-card details also remain. Multilingual snapshot coverage, the 201-bot load gate, the complete adversarial role/retry/hallucination matrix, live Telegram/Moonshot staging proof, and the dated security audit remain open. See [../../START_HERE.md](../../START_HERE.md).
