...
Receptionist advance selection creates a replay-safe audited owner-bot notification without collecting or mutating financial values. Barber callbacks now reauthorize the exact active same-shop membership and expose only that barber's current queue, daily earnings, payout rows, and advance/deduction history. A localized five-minute customer reminder is queued and audited at most once for an assigned confirmed booking. Owner callbacks repeat active ownership authorization and provide an explicit owned-shop selector, business/shop daily and monthly reports, barber performance, advance history, recent audit activity, and subscription status. Advance approval requires an owned shop, active barber, open cash shift, positive amount, and an explicit confirmation before the existing atomic grant service disburses once. Draft payout approval and approved payout payment revalidate the run and optional open cash shift, display the selected scope/period/amount, and require explicit confirmation before the existing service applies advance deductions and pays once; callback replay returns the same result. Master callbacks, customer contact-share/name capture, and full notification cards remain. The receptionist bot does not weaken the Phase 2 rule that only owners/platform administrators may grant advances. Multilingual snapshot coverage, the 201-bot load gate, the complete adversarial role/retry/hallucination matrix, live Telegram/Moonshot staging proof, and the dated security audit remain open. See [../../START_HERE.md](../../START_HERE.md).
